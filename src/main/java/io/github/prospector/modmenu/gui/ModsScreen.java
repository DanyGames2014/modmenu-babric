...
import io.github.prospector.modmenu.gui.widget.Controller;
import io.github.prospector.modmenu.gui.widget.DescriptionListWidgetModMenu;
import io.github.prospector.modmenu.gui.widget.ModListWidgetModMenu;
import io.github.prospector.modmenu.gui.widget.TextFieldAccess;
import io.github.prospector.modmenu.gui.widget.TexturedButtonWidget;
import io.github.prospector.modmenu.gui.widget.entries.ModListEntry;
import io.github.prospector.modmenu.util.DrawingUtil;
import io.github.prospector.modmenu.util.ScreenUtil;
import io.github.prospector.modmenu.util.TranslationUtil;
import io.github.prospector.modmenu.util.mod.Mod;
...
public class ModsScreen extends Screen implements Controller {
	...
	private static final String TOGGLE_FILTER_OPTIONS = TranslationStorage.getInstance().get("modmenu.toggleFilterOptions");
	private static final String CONFIGURE = TranslationStorage.getInstance().get("modmenu.configure");
	...
	public ModsScreen(Screen previousScreen) {
		this.title = TranslationStorage.getInstance().get("modmenu.title");
...
//			this.modList.handleMouse();
	...
	public void init() {
		...
		this.modList = new ModListWidgetModMenu(this.minecraft, paneWidth, this.height, paneY, this.height - 36, ModMenuConfig.COMPACT_LIST.getValue() ? 23 : 36, this.searchBox.getText(), this.modList, this);
		this.modList.setX(0);
		modList.reloadFilters();

		this.descriptionListWidget = new DescriptionListWidgetModMenu(this.minecraft, paneWidth, this.height, RIGHT_PANE_Y + 60, this.height - 36, DrawingUtil.fontHeight + 1, this);
		...
		ButtonWidget configureButton = new TexturedButtonWidget(DESCRIPTION_LIST, width - 24, RIGHT_PANE_Y, 20, 20, 0, 0, 20, CONFIGURE_BUTTON_LOCATION, 32, 64) {
			...
			public void render(Minecraft minecraft, int mouseX, int mouseY) {
				...
				if (modScreenErrors.containsKey(modId)) {
					...
					this.tooltip = TranslationStorage.getInstance().get("modmenu.configure.error", modId, modId) + "\n\n" /*+ Formatting.RED*/ + e.toString();
				} else {
		...
		ButtonWidget websiteButton = new ButtonWidget(WEBSITE, rightPaneX + (urlButtonWidths / 2) - (cappedButtonWidth / 2), RIGHT_PANE_Y + 36, Math.min(urlButtonWidths, 200), 20, TranslationStorage.getInstance().get("modmenu.website")) {
			@Override
			public void render(Minecraft minecraft, int mouseX, int mouseY) {
		...
		ButtonWidget issuesButton = new ButtonWidget(ISSUES, rightPaneX + urlButtonWidths + 4 + (urlButtonWidths / 2) - (cappedButtonWidth / 2), RIGHT_PANE_Y + 36, Math.min(urlButtonWidths, 200), 20, TranslationStorage.getInstance().get("modmenu.issues")) {
			@Override
			public void render(Minecraft minecraft, int mouseX, int mouseY) {
				visible = selected != null;
		...
		this.buttons.add(new ButtonWidget(SORTING, filtersX, 45, sortingWidth, 20, sortingText) {
			...
			public void render(Minecraft minecraft, int mouseX, int mouseY) {
				...
				this.text = ModMenuConfig.SORTING.getValueLabel();
				super.render(minecraft, mouseX, mouseY);
			}
		});
		this.buttons.add(new ButtonWidget(LIBRARIES, filtersX + sortingWidth + 2, 45, showLibrariesWidth, 20, showLibrariesText) {
			...
			public void render(Minecraft minecraft, int mouseX, int mouseY) {
				visible = filterOptionsShown;
				this.text = ModMenuConfig.SHOW_LIBRARIES.getValueLabel();
				super.render(minecraft, mouseX, mouseY);
			}
		});
		...
		this.buttons.add(new ButtonWidget(MODS_FOLDER, this.width / 2 - 154, this.height - 28, 150, 20, TranslationStorage.getInstance().get("modmenu.modsFolder")));
		this.buttons.add(new ButtonWidget(DONE, this.width / 2 + 4, this.height - 28, 150, 20, TranslationStorage.getInstance().get("gui.done")));
	...
	public void buttonClicked(ButtonWidget button) {
		...
		case WEBSITE: {
			...
			minecraft.setScreen(new ConfirmChatLinkScreen(this, mod.getWebsite(), WEBSITE) {
		...
		case ISSUES: {
			final Mod mod = Objects.requireNonNull(selected).getMod();
			minecraft.setScreen(new ConfirmChatLinkScreen(this, mod.getIssueTracker(), ISSUES) {
	...
	public void render(int mouseX, int mouseY, float delta) {
		...
		this.drawCenteredTextWithShadow(this.textRenderer, this.title, this.modList.getWidth() / 2, 8, 16777215);
		if (!ModMenuConfig.CONFIG_MODE.getValue()) {
		...
		if (selectedEntry != null) {
			...
			if (mouseX > x + imageOffset && mouseY > RIGHT_PANE_Y + 1 && mouseY < RIGHT_PANE_Y + 1 + DrawingUtil.fontHeight && mouseX < x + imageOffset + textRenderer.getWidth(trimmedName)) {
				setTooltip(Collections.singletonList(TranslationStorage.getInstance().get("modmenu.modIdToolTip", mod.getId())));
			}
			...
			if (!names.isEmpty()) {
				...
				DrawingUtil.drawWrappedString(TranslationStorage.getInstance().get("modmenu.authorPrefix", authors), x + imageOffset, RIGHT_PANE_Y + 2 + lineSpacing * 2, paneWidth - imageOffset - 4, 1, 0x808080);
			}
		}
	...
	@Override
	public void confirmed(boolean result, int id) {
		if (id < MODS_LIST_CONFIRM_ID_OFFSET) {
			super.confirmed(result, id);
