...
public class ModListEntry extends DrawContext implements ModMenuEntryListWidget.Entry {
...
//	private static final String ERROR_ICON = new Identifier("minecraft", "textures/gui/world_selection.png");
	...
	public ModListEntry(Mod mod, ModListWidgetModMenu list) {
		this.mod = mod;
		this.list = list;
		this.client = Minecraft.INSTANCE;
	}

	@Override
	public void render(int index, int x, int y, int rowWidth, int rowHeight, Tessellator bufferBuilder, int mouseX, int mouseY, boolean hovered) {
		x += getXOffset();
		rowWidth -= getXOffset();
		...
		if (!(this instanceof ParentEntry) && ModMenuConfig.QUICK_CONFIGURE.getValue() && (this.list.getParent().getModHasConfigScreen().get(modId) || this.list.getParent().modScreenErrors.containsKey(modId))) {
			...
			if (/*this.client.options.touchscreen ||*/ hovered) {
				...
				if (this.list.getParent().modScreenErrors.containsKey(modId)) {
					this.client.textureManager.bindTexture(this.client.textureManager.getTextureId(ERROR_ICON));
					...
					if (hoveringIcon) {
						...
						this.list.getParent().setTooltip(this.client.textRenderer.split(TranslationStorage.getInstance().get("modmenu.configure.error", modId, modId) + "\n\n" + /*Formatting.RED +*/ e.toString(), 175));
					}
				} else {
					this.client.textureManager.bindTexture(this.client.textureManager.getTextureId(MOD_CONFIGURATION_ICON));
	...
	public void openConfig() {
		client.setScreen(ModMenu.getConfigScreen(mod.getId(), list.getParent()));
	}
