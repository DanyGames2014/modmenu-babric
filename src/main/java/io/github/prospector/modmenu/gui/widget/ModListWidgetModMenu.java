...
import io.github.prospector.modmenu.gui.widget.entries.ModMenuEntryListWidget;
...
public class ModListWidgetModMenu extends ModMenuEntryListWidget implements AutoCloseable {
	...
	public ModListWidgetModMenu(Minecraft client, int width, int height, int y1, int y2, int entryHeight, String searchTerm, ModListWidgetModMenu list, ModsScreen parent) {
		...
		this.scrollAmount = parent.getScrollPercent() * Math.max(0, this.getMaxScroll() - (this.bottom - this.top - 4));
		this.clampScrolling();
	}
	...
	public void scroll(int amount) {
		super.scroll(amount);
		int denominator = Math.max(0, this.getMaxScroll() - (this.bottom - this.top - 4));
		if (denominator <= 0) {
			this.parent.updateScrollPercent(0);
		} else {
			this.parent.updateScrollPercent(getScrollAmount() / Math.max(0, this.getMaxScroll() - (this.bottom - this.top - 4)));
		}
	}
...
//			mods.addAll(TestModContainer.getTestModContainers());
		...
		if (getScrollAmount() > Math.max(0, this.getMaxScroll() - (this.bottom - this.top - 4))) {
			scroll(Math.max(0, this.getMaxScroll() - (this.bottom - this.top - 4)));
		}
	}
	...
	public boolean mouseClicked(int double_1, int double_2, int int_1) {
		...
		if (!this.isMouseInList(double_1, double_2)) {
			...
			if (entry != null) {
				...
				this.headerClicked((int) (double_1 - (double) (this.left + this.width / 2 - this.getRowWidth() / 2)), (int) (double_2 - (double) this.top) + (int) this.getScrollAmount() - 4);
				return true;
			}
	...
	public final ModListEntry getEntryAtPos(double x, double y) {
		...
		return x < (double) this.getScrollbarPosition() && x >= (double) getRowLeft() && x <= (double) (getRowLeft() + getRowWidth()) && index >= 0 && int_5 >= 0 && index < this.getEntryCount() ? this.entries.get(index) : null;
	}
	...
	public int getRowWidth() {
		return this.width - (Math.max(0, this.getMaxScroll() - (this.bottom - this.top - 4)) > 0 ? 18 : 12);
	}
