...
public abstract class ModMenuEntryListWidget extends EntryListWidget implements ListWidgetHelper {
	...
	public int getEntryAt(int x, int y) {
		...
		int k = j - i;
		int l = j + i;
		int m = MathHelper.floor(y - (double)this.top) - this.headerHeight + this.getScrollAmount() - 4; // convertToBlockCoord
		int n = m / this.itemHeight;
		return x < (double)this.getScrollbarPosition() && x >= (double)k && x <= (double)l && n >= 0 && m >= 0 && n < this.getEntryCount() ? n : -1;
	}
